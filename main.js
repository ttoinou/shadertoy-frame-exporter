...
FrameExporter.prototype.createUi = function() {
    ...
    this.fpsInput = this.createInput('fps', 'number', 60);
    this.secondsInput = this.createInput('seconds', 'number',340);
...
FrameExporter.prototype.fixSoundCloud = function(){
    ...
    gShaderToy.mEffect.mPasses.forEach(function mPass(pass) {
        ...
        pass.mInputs.forEach(function mInput(input,slot){
            ...
            {
                var url = input.mInfo.mSrc;
                var k = url.lastIndexOf('/');
                var secret_key = url.substring(k+1,url.length);
                var secretTrack = false;
                if( secret_key.substring(0,2) == 's-')
                {
                    secretTrack = true;
